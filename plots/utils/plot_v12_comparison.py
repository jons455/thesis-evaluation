"""
Plot focused comparison for Incremental SNN model (formerly v12): Run 1 vs Run 2, and Run 1 baseline vs all models.

This script creates normalized histogram plots (0-1 range, percentages):
1. Run 1 vs Run 2 comparison for Incremental SNN (control and neuromorphic metrics)
...
def plot_incremental_snn_run_comparison_histogram(
    ...
    """
    Plot histogram comparison between Run 1 and Run 2 for Incremental SNN model.
    ...
    plt.suptitle(
        f"Incremental SNN Model: Run 1 vs Run 2 Comparison - {title_suffix}",
        fontsize=14,
        fontweight="bold",
    )
    plt.tight_layout()

    output_path = output_dir / f"incremental_snn_run1_vs_run2_{metric_category}_histogram.png"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Plot Incremental SNN comparison: Run 1 vs Run 2, and Run 1 baseline vs models"
    )
    parser.add_argument(
        "--results-dir",
        type=Path,
    ...
    # Load Incremental SNN (v12) for both runs
    v12_run1_path = run1_dir / "v12_incremental_best_model.json"
    v12_run2_path = run2_dir / "v12_incremental_best_model.json"

    if not v12_run1_path.exists():
        print(f"Error: {v12_run1_path} not found!")
        return 1
    if not v12_run2_path.exists():
        print(f"Error: {v12_run2_path} not found!")
        return 1

    v12_run1_data = load_benchmark_json(v12_run1_path)
    v12_run2_data = load_benchmark_json(v12_run2_path)

    print(f"\nLoaded Incremental SNN model from both runs")
    ...
    # 1. Incremental SNN Run 1 vs Run 2 - Control metrics
    print("  Creating Incremental SNN Run 1 vs Run 2 control metrics histogram...")
    plot_incremental_snn_run_comparison_histogram(v12_run1_data, v12_run2_data, args.output_dir, "control")

    # 2. Incremental SNN Run 1 vs Run 2 - Neuromorphic metrics
    print("  Creating Incremental SNN Run 1 vs Run 2 neuromorphic metrics histogram...")
    plot_incremental_snn_run_comparison_histogram(v12_run1_data, v12_run2_data, args.output_dir, "neuromorphic")
